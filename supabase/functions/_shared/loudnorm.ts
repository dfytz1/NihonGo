 ...
 * On hosted Supabase Edge, ffmpeg is usually absent → bytes unchanged, loudnorm_applied false.
 *
 * Set DISABLE_AUDIO_LOUDNORM=1 to skip processing (e.g. debugging).
 */
export type NormalizeMp3Result = {
  bytes: Uint8Array;
  /** True only when ffmpeg loudnorm produced new audio successfully */
  loudnorm_applied: boolean;
};

export async function normalizeMp3ForStorage(
  input: Uint8Array,
): Promise<NormalizeMp3Result> {
  if (Deno.env.get("DISABLE_AUDIO_LOUDNORM") === "1") {
    return { bytes: input, loudnorm_applied: false };
  }
  ...
  try {
    ...
    return { bytes: outBuf, loudnorm_applied: true };
  } catch (e) {
